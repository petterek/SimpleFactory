...
namespace Itas.Service.SimpleFactory
{
    public delegate object ObjectFactory(IServiceProvider serviceProvider, object[] arguments);
    public static class ActivatorUtilities
    {
        private const string AmbiguousConstructorMatch = "Multiple constructors accepting all given argument types have been found in type '{0}'. There should only be one applicable constructor.";
        private const string NoConstructorMatch = "Unable to locate suitable constructor for type '{0}'. Ensure the type is concrete and all parameters are accepted by a constructor.";
        private const string CannotResolveService = "Unable to resolve service for type '{0}' while attempting to activate '{1}'.";
        ...
        public static object CreateInstance(IServiceProvider provider, Type instanceType, params object[] parameters)
        {
            ...
            if (bestMatcher == null)
            {
                throw new InvalidOperationException(string.Format(NoConstructorMatch, instanceType));
            }
        ...
        private static object GetService(IServiceProvider sp, Type type, Type requiredBy, bool isDefaultParameterRequired)
        {
            ...
            if (service == null && !isDefaultParameterRequired)
            {
                throw new InvalidOperationException(
                    string.Format(CannotResolveService, type, requiredBy));
            }
            return service;
        }
            ...
            out int?[] parameterMap)
        {
            ...
            foreach (var constructor in instanceType.GetTypeInfo().DeclaredConstructors)
            {
                ...
                if (TryCreateParameterMap(constructor.GetParameters(), argumentTypes, out tempParameterMap))
                {
                    ...
                    {
                        throw new InvalidOperationException(string.Format(AmbiguousConstructorMatch, instanceType));
                    }
                ...
                for (var index = 0; index < _parameters.Length; index++)
                {
                    if (_parameterValuesSet[index] == false)
                    {
                        ...
                        if (value == null)
                        {
                            ...
                            {
                                throw new InvalidOperationException(string.Format(CannotResolveService,
